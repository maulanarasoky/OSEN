...
class EditStudent : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        showScore(parcel?.id, parcel?.teacher_id, parcel?.className)
        countScore()
        ...
        btnAbsent.setOnClickListener {
            if (spinnerAbsent.selectedItem.toString() == "Pilih") {
        ...
        inputScore.setOnClickListener {
            ...
            intent.putExtra(InputScore.data, parcel)
    ...
    private fun showAbsent() {
        ...
        database.use {
            val result = select(Absent.TABLE_ABSENT).whereArgs(
                ...
                "teacher_id" to parcel?.teacher_id.toString(),
                "student_id" to parcel?.id.toString()
            )
    ...
    private fun checkDailyAbsent(
        ...
        database.use {
            val result = select(AbsentOfDay.TABLE_ABSENTOFDAY).whereArgs(
                "(STUDENT_ID = {student_id}) AND (CLASS = {class_name}) AND (TEACHER_ID = {teacher_id}) AND (DATE = {date}) LIMIT 1",
        ...
        if (checkDailyAbsent) {
            database.use {
                val queryUpdate = update(
                    ...
                    "(STUDENT_ID = {student_id}) AND (CLASS = {class_name}) AND (TEACHER_ID = {teacher_id}) AND (DATE = {date})",
                    "student_id" to student_id.toString(),
                    "class_name" to class_name.toString(),
                    "teacher_id" to teacher_id.toString(),
                    "date" to date.toString()
    ...
    private fun absent(student_id: Int?, teacher_id: String?, class_name: String?) {
        ...
        database.use {
            val queryUpdate =
                update(Absent.TABLE_ABSENT, columnAdd to add, columnSubtract to subtract).whereArgs(
    ...
    private fun showScore(student_id: Int?, teacher_id: String?, class_name: String?) {
        scoreList.clear()
        database.use {
    ...
    private fun countScore() {
        ...
        val ass1: Float? =
            scoreList[0].persentaseAssessment1?.times(scoreList[0].assessment1!!)?.div(100)
                ?.toFloat()
        val ass2: Float? =
            scoreList[0].persentaseAssessment2?.times(scoreList[0].assessment2!!)?.div(100)
                ?.toFloat()
        val ass3: Float? =
            scoreList[0].persentaseAssessment3?.times(scoreList[0].assessment3!!)?.div(100)
                ?.toFloat()

        val result: Float? = uts?.plus(uas!!)?.plus(ass1!!)?.plus(ass2!!)?.plus(ass3!!)

        score.text = result.toString()
    }

    private fun countPersentaseKehadiran() {
        val totalAbsen: Float? =
            totalAlfa?.plus(totalSakit!!)?.plus(totalIzin!!)?.plus(totalHadir!!)?.toFloat()
