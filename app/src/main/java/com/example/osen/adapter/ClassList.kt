...
class ClassList(private val items : List<Classroom>) : RecyclerView.Adapter<ClassList.ViewHolder>() {
    ...
    class ViewHolder(override val containerView : View) : RecyclerView.ViewHolder(containerView),
        ...
        fun bindItem(items : Classroom){
            className.text = items.className

            Glide.with(itemView.context).load(containerView.resources.getDrawable(items.classImage!!.toInt())).apply(RequestOptions.overrideOf(500,500)).into(classImage)
        }
    }
}